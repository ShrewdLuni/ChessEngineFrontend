...
export const MoveHint = ({ type, index, isFlipped, onClick } : MoveHintProps) => {
  let {row, col} = helpers.getRowAndColFromIndex(isFlipped ? 63 - index : index);

  return (
    <div style={{top: `${row * 12.5}%`, left: `${col * 12.5}%`}} className="tile absolute bg-contain z-1" onClick={onClick}>
