...
export const Sidebar = ({ evaluation, bestMove, moveHistory, flip, FEN, SetFEN, engineSetPosition, isPromotion, handleMove, promotionOptions, isGameOver, isSettings, setIsSettings, toggleRematch, toggleSettings } : SidebarProps) => {
  return (
    <div className={cn("boardInfo","flex flex-col justify-start gap-4 text-center bg-[#1a1e23] text-white font-bold font-mono text-sm ml-2 overflow-y-scroll no-scrollbar")}>
      {true && <Evaluation evaluation={evaluation} bestMove={bestMove} flip={flip} setIsSettings={setIsSettings} isSettings={isSettings}/>}
